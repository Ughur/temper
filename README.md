...
Temper is an unstyled drawer component for React that can be used as a Dialog replacement on tablet and mobile devices. You can read about why and how it was built [here](https://emilkowal.ski/ui/building-a-drawer-component).
...
Contains all parts of a dialog. Use `shouldScaleBackground` to enable background scaling, it requires an element with `[data-temper-drawer-wrapper]` data attribute to scale its background.
...
`repositionInputs`: When `true` Temper will reposition inputs rather than scroll then into view if the keyboard is in the way. Setting it to `false` will fall back to the default browser behavior.

`noBodyStyles`: When `true` the `body` doesn't get any styles assigned from Temper.
...
`[data-temper-no-drag]`: When interacting with an element with this data attribute, the drawer won't be dragged.
...
CSS in this library is inserted by JavaScript into `<head>` element by default. If you're having issue with it such as [CSP restriction](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/style-src), you can use `temper/style.css` CSS as a workaround to apply the styles.
