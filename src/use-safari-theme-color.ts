...
export function useSafariThemeColor(
  ...
  React.useEffect(() => {
    ...
    const backgroundColor = documentElementStyle
      .getPropertyValue('--temper-overlay-background')
      .split(',')
      .map((c) => Number(c));

    const overlayColor = documentElementStyle.getPropertyValue('--temper-overlay-background-end');
      ...
      function draw(timeStamp: number) {
        ...
        if (overlay.current && colors && !releaseExit) {
          if (
            ...
            drawer.current.getAttribute('temper-clicked-outside') !== 'true'
          ) {
